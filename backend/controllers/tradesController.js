  ...
  try {
    ...
    if (!user) {
      const error = new Error(
        `User with id: ${req.body.userId} not found in the database`
      );
      error.validationErrors = errors.errors;
      error.statusCode = 422;
      throw error;
...
const deleteTrade = asyncHandler(async (req, res, next) => {
  ...
  if (trade.userId.id !== req.body.userId) {
    res.status(401);
    throw new Error('User who does not own the trade cannot delete the trade');
  }

  if (trade.executionDate < new Date()) {
    res.status(401);
    throw new Error(
      'Trades that have Execution Date in the past cannot be delete'
    );
  }
